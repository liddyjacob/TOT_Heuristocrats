...
from ai.shitutils import get_tiles
...
from ai.heuristocrats.exploration import initialize_exp_weight_map, multi_aggregate, exp_render
# notes
"""
...
def run(world_state, players, team_idx):
    ...
    commands = [{"id":vils[0]["id"],"command":"m","arg":DIRECTIONS['UP']}]

    reflected_map = reflect(world_state)

    exp_weights = initialize_exp_weight_map(world_state)
    print(exp_render(multi_aggregate(exp_weights, 6)))
    #render(reflected_map)
